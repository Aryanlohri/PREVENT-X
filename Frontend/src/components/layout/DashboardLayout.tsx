...
import {
  LayoutDashboard, Activity, UtensilsCrossed, Bell, UserCircle, LogOut, Heart, Menu, X, Settings, ShieldAlert, Apple, Droplets
} from "lucide-react";
...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  const initials = userProfile?.full_name ? userProfile.full_name.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() : "U";
  ...
  const navItems = [
    ...
    { icon: Apple, label: "AI Lifestyle Plan", path: "/ai-plan" },
      ...
      <aside className="hidden lg:flex flex-col w-64 glass-card border-r border-border/50 fixed inset-y-0 left-0 z-30">
        ...
        <nav className="flex-1 px-4 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={cn("flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]", active ? "bg-primary/10 text-primary shadow-sm" : "text-muted-foreground hover:bg-accent hover:text-foreground")}>
          ...
          <aside className="absolute inset-y-0 left-0 w-72 glass-card border-r border-border/50 flex flex-col">
            ...
            <nav className="flex-1 px-4 space-y-1">
              {navItems.map((item) => {
                const active = location.pathname === item.path;
                return (
                  <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} className={cn("flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]", active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent hover:text-foreground")}>
                    <item.icon className="h-5 w-5" />
                    {item.label}
                  </Link>
                );
        ...
        <header className="sticky top-0 z-20 glass-card border-b border-border/50 px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h2 className="font-heading font-semibold text-foreground">
              {t(lang, "welcomeBack")} {firstName && <span className="gradient-text">{firstName}</span>}
            </h2>
          </div>
