  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="font-heading text-xl sm:text-2xl font-bold text-foreground">{t(lang, "riskPredictionTitle")}</h1>
          <p className="text-xs sm:text-sm text-muted-foreground">{t(lang, "riskPredictionDesc")}</p>
        </div>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full sm:w-auto">
          <TabsList className="grid w-full grid-cols-2 rounded-xl bg-muted/50 p-1 h-11">
            <TabsTrigger value="dashboard" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm text-[11px] sm:text-xs md:text-sm transition-all">
              {t(lang, "healthRiskDashboard")}
            </TabsTrigger>
            <TabsTrigger value="symptoms" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm text-[11px] sm:text-xs md:text-sm transition-all">
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
        <h3 className="font-heading font-semibold text-foreground mb-4 text-sm sm:text-base">{t(lang, "conditionRiskAnalysis")}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {diseaseRisks.map((d, i) => (
            <div key={i} className="glass-card-hover rounded-2xl p-4 sm:p-5">
              <div className="flex items-center justify-between mb-1">
                <h4 className="font-heading font-semibold text-foreground text-sm sm:text-base">{d.name}</h4>
                <TrendingUp className={`h-4 w-4 shrink-0 ${d.risk >= 60 ? "text-destructive" : d.risk >= 40 ? "text-warning" : "text-success"}`} />
              </div>
              <p className="text-[11px] sm:text-sm text-muted-foreground mb-3">{t(lang, "riskLevel")}: <span className="font-bold text-foreground">{d.risk}%</span></p>
              <div className="w-full h-1.5 sm:h-2 rounded-full bg-muted mb-4 overflow-hidden">
              ...
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] sm:text-xs font-semibold text-foreground uppercase tracking-wider">{t(lang, "contributingFactors")}</span>
              </div>
              <ul className="space-y-1.5 sm:space-y-1">
                {d.factors.map((f, j) => (
                  <li key={j} className="text-[11px] sm:text-xs text-muted-foreground flex items-start gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-secondary inline-block mt-0.5 shrink-0" />
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card rounded-2xl p-4 sm:p-6">
        <h3 className="font-heading font-semibold text-foreground mb-4 text-sm sm:text-base">{t(lang, "keyContributing")}</h3>
        {recentLogs.length > 0 ? (
            ...
            {keyFactors.map((f, i) => (
              <div key={i} className="glass-card rounded-xl p-3 sm:p-4">
                <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-2 mb-2">
                  <f.icon className={`h-4 w-4 sm:h-5 sm:w-5 ${f.color}`} />
                  <span className={`text-[9px] sm:text-[10px] font-semibold px-2 py-0.5 rounded-full border w-fit ${f.isHigh ? "border-destructive/30 text-destructive bg-destructive/5" : "border-warning/30 text-warning bg-warning/5"}`}>
                    {f.impact} {t(lang, "impact")}
                  </span>
                </div>
                <p className="text-xs sm:text-sm font-semibold text-foreground mt-1 sm:mt-2 truncate">{f.label}</p>
                <div className="w-full h-1 sm:h-1.5 rounded-full bg-muted mt-2 mb-1 overflow-hidden">
                  <div className="h-full rounded-full bg-secondary" style={{ width: `${f.score}%` }} />
                </div>
                <div className="flex flex-col xs:flex-row justify-between text-[9px] sm:text-[10px] text-muted-foreground">
                  <span>{t(lang, "score")}: {f.score}</span>
